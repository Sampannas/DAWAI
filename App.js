...
import { View, Text, Pressable, StyleSheet, ScrollView, ActivityIndicator, StatusBar, Animated, Dimensions } from 'react-native';
...
// Channel Configuration - FL Studio style
const CHANNELS = [
  {
    id: 'drums',
    name: 'Drums',
...
// --- (Place this function above your PatternBlock component) ---

// This function takes a pattern array (e.g., [true, false, true, true])
// and returns an array of consecutive active step indices (e.g., [[0], [2, 3]]).
const groupConsecutiveSteps = (pattern) => {
  ...
  // Add the last group if the pattern ends with active steps
  if (currentGroup.length > 0) {
    groups.push(currentGroup);
  }
...
// --- Pattern Block Component (FL Studio style) ---
// --- Pattern Block Component (FL Studio style) ---
// This component now renders a single consecutive block of notes.
const PatternBlock = ({ group, stepWidth, trackColor, isPlaying, activeStep }) => {
  ...
  const blockWidth = ((endStep - startStep + 1) * stepWidth) - 4; // -4 for padding
  ...
  return (
    <View style={[
      ...
      <View style={styles.patternDots}>
        {group.map((stepIndex) => {
          // Calculate the dot's position relative to its block
  ...
  return (
    <View style={[styles.channelHeader, { backgroundColor: channel.color + '15' }]}>
      ...
      <View style={styles.channelControls}>
        <Pressable 
          style={[styles.channelButton, isSolo && styles.channelButtonActive]}
          onPress={onSolo}
        >
        ...
        <Pressable 
          style={[styles.channelButton, isMuted && styles.channelButtonMuted]}
          onPress={onMute}
        >
...
export default function DrumMachine() {
  ...
  useEffect(() => {
    // Function to lock the screen to landscape
    ...
    // Optional cleanup: Unlock orientation when the component is closed
    return () => {
  ...
  // Check if track should play (considering mute/solo/volume)
  const shouldPlayTrack = (trackIndex) => {
    ...
    if (anySolo) {
      return channelState.solo && !channelState.muted;
    }
  ...
  const clearPattern = () => {
    setGrid(Array(ALL_SAMPLES.length).fill(null).map(() => Array(NUM_STEPS).fill(false)));
  };

  const randomizePattern = () => {
    setGrid(
      Array(ALL_SAMPLES.length).fill(null).map(() => 
        Array(NUM_STEPS).fill(false).map(() => Math.random() > 0.75)
      )
    );
  };

  const handleChannelMute = (channelId) => {
  ...
  const handleChannelSolo = (channelId) => {
    setChannelStates(prev => ({
      ...prev,
      [channelId]: { ...prev[channelId], solo: !prev[channelId].solo }
    }));
  };

  const handleChannelVolume = (channelId, volume) => {
    setChannelStates(prev => ({
      ...prev,
      [channelId]: { ...prev[channelId], volume }
    }));
  };
  ...
  const toggleChannelExpand = (channelId) => {
    setChannelStates(prev => ({
      ...prev,
      [channelId]: { ...prev[channelId], expanded: !prev[channelId].expanded }
    }));
  };
      ...
      <View style={styles.compactHeader}>
        <View style={styles.headerLeft}>
          ...
          <View style={styles.headerTitleContainer}>
            <Text style={styles.compactTitle}>Rhythm Studio</Text>
              ...
              {Array(NUM_STEPS).fill(0).map((_, i) => (
                <View key={i} style={[styles.timelineStep, { width: stepWidth }]}>
                  <Text style={[
                    ...
                    activeStep === i && styles.timelineStepNumberActive
                  ]}>
                    {i + 1}
                  </Text>
                  <View style={[
                    styles.timelineMarker,
                    (i + 1) % 4 === 0 && styles.timelineMarkerAccent,
                    activeStep === i && styles.timelineMarkerActive
                  ]} />
                </View>
              ))}
            </View>
        ...
        >
          <View style={styles.playlistContent}>
            {CHANNELS.map((channel, channelIndex) => {
              ...
              const channelTracks = ALL_SAMPLES
              ...
              return (
                <View key={channel.id} style={styles.channelSection}>
                  ...
                  {channelState.expanded && channelTracks.map(({ sample, index }) => {
                    const isMuted = channelState.muted || (
                      Object.values(channelStates).some(s => s.solo) && !channelState.solo
                    );
                            ...
                            {Array(NUM_STEPS).fill(0).map((_, i) => (
                              <View 
                                ...
                                style={[
                                  ...
                                  { 
                                    left: i * stepWidth,
                                    backgroundColor: (i + 1) % 4 === 0 ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.03)'
                                  }
                                ]} 
                              />
                            ))}
                            ...
                            {grid[index].map((isActive, stepIndex) => (
                              <Pressable
                                ...
                                style={[
                                  ...
                                  { 
                                    width: stepWidth - 4,
                                    left: stepIndex * stepWidth + 2,
                                  }
                                ]}
                                onPress={() => handlePressCell(index, stepIndex)}
                              />
                            ))}
                          </View>
                          
                          {/* Pattern Block */}
                          {/* Pattern Blocks (now renders multiple for consecutive groups) */}
  ...
  />
                          ...
                          {activeStep >= 0 && !isMuted && (
                            <View style={[
                              styles.playheadLine,
                              { 
                                left: activeStep * stepWidth + stepWidth / 2,
                                backgroundColor: sample.channelColor
                              }
                            ]} />
                          )}
                        </View>
...
const THEME = {
  ...
  playheadGlow: 'rgba(0, 212, 255, 0.8)',
};
...
const styles = StyleSheet.create({
  ...
  stepButtons: {
    ...
    left: 0,
    bottom: 0,
    width: NUM_STEPS * 50, // Explicitly set the width
  },
  stepButton: {
    position: 'absolute',
    top: 0,
